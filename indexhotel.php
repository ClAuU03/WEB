...
<html lang="en">
  <?php 
      ...
      khas.khasKODE");
      $restorantinfo = mysqli_query ($connection, "select * from restorant,khas where restorant.khasKODE = 
      khas.khasKODE");
      ...
      $provinsiinfo = mysqli_query ($connection, "select * from provinsi");
...
<div class="col-sm-8"  style="background: #2f3640" >         <!-- sm untuk kalo dibuka di hape , untuk menyesuaikan ukuran kolom di hape -->
    <?php 
      ...
      {
         while ($row=mysqli_fetch_array($hotelinfo))
         {   ?>
    <div class="d-flex">
      
    <img style="width: 200px; height: 100%;"src="admin/images/<?php echo $row["hotelFOTO"]?>" alt="No Image">
        <div class="flex-shrink-0">
        <!-- height: 100% ngikutin widthnya -->
        </div> <!--penutup flexshrink -->
        <div class="flex-grow-1" style="margin-left: 20px; color: white;">
            <h2><?php echo $row["hotelNAMA"]?> <small class="text-muted" style="font-size: 15px;">
              <i style="color: #f9ca24"><?php echo $row ["provinsiNAMA"] ?><span>-</span> <?php echo $row ["provinsiIBUKOTA"]?></i></small></h2>
            <p><?php echo substr($row ["hotelKET"],0,150)?> ...</p>
            <a href="readmorehotel.php?detail=<?php echo urlencode($row["hotelKODE"])?>" class="btn btn-warning btn-outline-light">Read More </a>
...
<h1 class="card-title" style="margin-bottom: 40px;">Provinsi</h1>
  <?php 
    if (mysqli_num_rows($provinsiinfo) > 0) {
        $count = 0; // Tambahkan variabel hitungan

        while ($row = mysqli_fetch_array($provinsiinfo)) {
            if ($count % 3 == 0) { // Baris baru setiap tiga gambar
  ?>
  ...
  ?>
                ...
                <div class="col-lg-4">
                  <div class="card" style="background:#f5f6fa;">
                    <img
                      src="admin/images/<?php echo $row["provinsiFOTO"]?>"
                      class="card-img-top"
                      alt="Waterfall"
                      style="height:100%;"
                    />
                    <div class="card-body">
                      ...
                      <p class="card-text"><?php echo $row ["provinsiIBUKOTA"] ?>
                      </p>
                      <a href="detailsprovinsi.php?detail=<?php echo urlencode($row["provinsiKODE"])?>" class="btn btn-warning btn-outline-light">Details</a>
                    </div>
                  </div>
                </div>
  <?php
            $count++;
            if ($count % 3 == 0) { // Tutup baris setiap tiga gambar
  ?>
                </div>
              </div>
            </div>
